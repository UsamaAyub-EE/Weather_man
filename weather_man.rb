...
module Report_gen
  def Report_gen.report_e_gen(data_array)
    max_temp = -99; max_temp_date = ""; min_temp = 1000; min_temp_date = ""; max_humid = -1; max_humid_date = ""
    ...
    end     #ymd is year month day
    ymd_max_temp = max_temp_date.split('-'); ymd_min_temp = min_temp_date.split('-'); ymd_max_humid = max_humid_date.split('-')
  ...
  def Report_gen.report_a_gen(data_array)
    sum_high_avg = 0; sum_low_avg = 0; sum_avg_humid = 0; count_high_avg = 0; count_low_avg = 0; count_avg_humid = 0 #initializations
    data_array.each do |row|
      if row[1].to_i.to_s == row[1]
        sum_high_avg += row[1].to_i; count_high_avg += 1       #second column contains max temperature
      end
      if row[3].to_i.to_s == row[3]
        sum_low_avg += row[3].to_i; count_low_avg += 1         #fourth column contains min temperature
      end
      if row[8].to_i.to_s == row[8]
        sum_avg_humid += row[8].to_i; count_avg_humid += 1     #ninth column contains mean humidity
      end
    end
  ...
  def Report_gen.report_c_gen(data_array,date)
    data_array.each do |row|
      if row[0].start_with?(date)
        ymd = row[0].split('-')
        print ymd[-1] + ' '; STDOUT.flush        #this is needed to remove the newline printed by print
        if row[1].to_i.to_s == row[1]
          high_temp = row[1].to_i          #second column contains max temperature
          high_temp.times do
            print "+".red; STDOUT.flush
          end
          puts " #{high_temp}C"
        end
        print ymd[-1] + ' '; STDOUT.flush
        if row[3].to_i.to_s == row[3]      #fourth column contains min temperature
          low_temp = row[3].to_i
          low_temp.times do
            print "+".blue; STDOUT.flush
          end
          puts " #{low_temp}C"
        end
      end
